...
Данные взял отсюда: https://www.kaggle.com/xxc025/apple2orange
"""
...
for root, dirs, files in os.walk(os.path.join(root, 'apple2orange/trainA')):
    for fil in files:
        apple_images.append(imread(os.path.join(root, fil)))

root = 'cycle_gan_datasets'
for root, dirs, files in os.walk(os.path.join(root, 'apple2orange/trainB')):
    for fil in files:
...
X_test_set, Y_test_set = next(iter(apple2orange_data_loader))
...
for i in range(1, 19, 2):
    plt.subplot(10, 2, i)
    plt.imshow(np.rollaxis(X_test_set[i].detach().cpu().numpy(), 0, 3))
    plt.subplot(10, 2, i+1)
    plt.imshow(np.rollaxis(Y_ret[i].detach().cpu().numpy(), 0, 3))
print('apple2orange_model X_Y_generator')
plt.show()
...
for i in range(n_images):
    imsave('cycle_gan_datasets/people2dicaprio/people/{0}.png'.format(i+1), img_as_ubyte(people_images[i]))
    imsave('cycle_gan_datasets/people2dicaprio/dicaprio/{0}.png'.format(i+1), img_as_ubyte(dicaprio_images[i]))
...
"""#people2dicaprio training"""

X_test_set, Y_test_set = next(iter(people2dicaprio_data_loader))

people2dicaprio_net = CycleGan(img_size=IMG_SIZE, mode='create')

people2dicaprio_net.train(data_loader=people2dicaprio_data_loader, epochs=1000, lr=0.0001, lmb=10, test_set=X_test_set)

people2dicaprio_net.save_models(path='cycle_gan_models/people2dicaprio_model')

people2dicaprio_net = CycleGan(img_size=IMG_SIZE, mode='load', path='cycle_gan_models/people2dicaprio_model')

Y_ret = people2dicaprio_net.G_X_Y(X_test_set.to(device))
...
for i in range(1, 19, 2):
    ...
    plt.subplot(10, 2, i+1)
    plt.imshow(np.rollaxis(Y_ret[i].detach().cpu().numpy(), 0, 3))
print('people2dicaprio_model X_Y_generator')
plt.show()

"""Я пытался сделать сеть, которая заменяет лица людей, на лицо Леонардо ДиКаприо, но получилось не очень. Я думаю, что проблема в том, что у меня был слишком маленький датасет (у меня не получилось больше спарсить, потому что яндекс картинки не выводят больше 1400 результатов), и возможно стоит обучить эту модель еще подольше, но я не успел к дедлайну(( Видно, что сеть пытается менять лица людей, но они напоминают лицо ДиКаприо очень отдаленно. Наверно, если бы я начал пораньше, спарсил бы еще картинок с гугла и поучил сеть подольше, то результаты были бы поинтереснее. Но на данный момент только так("""
